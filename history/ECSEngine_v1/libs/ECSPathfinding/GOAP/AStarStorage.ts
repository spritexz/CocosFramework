import { Pool } from "../../ECSEngine/Utils/Collections/Pool";
import { AStarNode } from "./AStar";
...
export class AStarStorage {
    ...
    public clear() {
        for (let i = 0; i < this._numOpened; i++) {
            Pool.free<AStarNode>(AStarNode, this._opened[i]);
            this._opened[i] = null;
        }

        for (let i = 0; i < this._numClosed; i++) {
            Pool.free<AStarNode>(AStarNode, this._closed[i]);
            this._closed[i] = null;
        }
