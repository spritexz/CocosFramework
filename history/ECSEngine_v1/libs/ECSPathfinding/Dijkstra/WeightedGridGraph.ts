...
import { List } from "../../ECSEngine/Utils/Linq/list";
import { IWeightedGraph } from "./IWeightedGraph";
import { WeightedPathfinder } from "./WeightedPathfinder";
...
export class WeightedGridGraph implements IWeightedGraph<Vector2> {
    ...
    public isNodePassable(node: Vector2): boolean {
        return !new List(this.walls).firstOrDefault(wall => JSON.stringify(wall) == JSON.stringify(node));
    }
