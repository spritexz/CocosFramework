import { Vector2 } from "../../ECSEngine/Math/Vector2";
import { PriorityQueue } from "../AStar/PriorityQueue";
import { PriorityQueueNode } from "../AStar/PriorityQueueNode";
...
export class WeightedPathfinder {
    public static search<T extends Vector2>(graph: IWeightedGraph<T>, start: T, goal: T, cameFrom: Map<string, T> = new Map<string, T>()) {
        let foundPath = false;
     ...
     * @param goal 
     */
    public static searchR<T extends Vector2>(graph: IWeightedGraph<T>, start: T, goal: T) {
     ...
     * @param goal 
     */
    public static recontructPath<T extends Vector2>(cameFrom: Map<string, T>, start: T, goal: T): T[] {
        let path = [];
        let current = goal;
        path.push(goal);
